Namespace Olvia.Devices.Octopus.Phasantlike
    Public Class Device
        ...
        Dim WithEvents COM As Comunicator = New Comunicator()
        ...
        Public Function GetDetector(ByVal DetectorNum, ByVal ParameterNum)
            Select Case ParameterNum
                Case 1
                    Return Detectors(DetectorNum).Amp
                Case 2
                    Return Detectors(DetectorNum).Angle
                Case 3
                    Return Detectors(DetectorNum).Distance
                Case 4
                    Return Detectors(DetectorNum).PhaseA
                Case 5
                    Return Detectors(DetectorNum).PhaseB
                Case 6
                    Return Detectors(DetectorNum).Speed
                Case 7
                    Return Detectors(DetectorNum).State
                Case Else
                    Return Nothing
            End Select
        ...
        Dim _flow As Boolean = False
        '------------------------------------------

        Public Property Flow_cntr As Boolean
            Get
                Return _flow
            End Get
        ...
        Public Event service_PackReaded(ByVal Pack As pack)
        Public Event service_PackWrited(ByVal Pack As pack)


        Private Sub COM_PackReaded(ByVal Pack As pack) Handles COM.PackReaded
            LastWDping = Now
            RaiseEvent service_PackReaded(Pack)
        End Sub

        Private Sub COM_PackWrited(ByVal Pack As pack) Handles COM.PackWrited
            '     LastWDping = Now
            RaiseEvent service_PackWrited(Pack)
        End Sub
